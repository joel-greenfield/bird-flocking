...
while t < T
    ...
    % distances = vecnorm(x(:,1:2) - obstacle_center, 2, 2);
    % in_range = x(distances <= 100); % 0 where bird n is not in range, 1 otherwise
    % 
    % 
    % %d = obstacle_center - in_range(:,1:2);
    % 
    ...
    %        p = [-d(2), d(1)];
    %        new_direction = obstacle_center + p;
